...
from preprocess import get_data, get_all
from utils import *
...
class ECMWrapper(Module):
    ...
    def __init__(self,hidden_size,state_size,emo_size,num_emotion,embedding,emotion_embedding,gru,device):
        ...
        self.read_g = Linear(self.hidden_size + self.hidden_size + self.hidden_size,self.emo_size)
        # write gate
        self.write_g = Linear(self.state_size, self.emo_size)
        # GRU output input dimensions = state_last + context + emotion emb + internal memory
        self.gru = gru
        self.emotion_embedding = emotion_embedding
        self.embedding = embedding
        # attention layer
        self.attn1 = Linear(self.hidden_size,self.hidden_size)
        self.attn2 = Linear(self.hidden_size,self.hidden_size)
        self.concat = Linear(self.hidden_size, 1)
    def forward(self,word_input,decoder_output,static_emotion_input,emotion_input,context_input,last_hidden,memory):
        ...
        if decoder_output is None:
            decoder_output = zeros(word_input.shape[1],self.hidden_size,dtype=torch.float,device = self.device)
        ...
        read_inputs = cat((last_word_embedding,decoder_output,context_input), dim = -1)
        # compute read input
        ...
        X = cat([context_input, last_word_embedding, emotion_input], dim = -1)
    ...
    def _write_internal_memory(self,emotion_input,rnn_output):
        ...
        """
        M_write = sigmoid(self.write_g(rnn_output))
...
class ECMGRU(Module):
    def __init__(self,hidden_size,static_emo_size,emo_size,n_layers = 1):
        ...
        self.emotion_u = Linear(static_emo_size + emo_size, hidden_size)
        self.emotion_r = Linear(static_emo_size + emo_size, hidden_size)
        self.emotion_c = Linear(static_emo_size + emo_size, hidden_size)
        # these two are generic GRU output that takes [step_input, last_hidden]
        self.generic_u = Linear(hidden_size + hidden_size + hidden_size + emo_size, hidden_size)
        self.generic_r = Linear(hidden_size + hidden_size + hidden_size + emo_size, hidden_size)
        
        # gate value computation
        self.generic_c = Linear(hidden_size + hidden_size + hidden_size + emo_size, hidden_size)
        self.n_layers = n_layers
        # starting from second layer, using the normal GRU
        self._cell = GRU(hidden_size, hidden_size,num_layers = n_layers - 1)
    def forward(self,step_input,last_hidden,emotion,internal_memory):
        ...
        emotion_input = cat([emotion,internal_memory],dim=-1)
        ...
        X = cat([step_input,hidden_0],dim = -1)
        u = self.generic_u(X)
        r = self.generic_r(X)
        ...
        # gate vector
        Xc = cat([step_input, hidden_0 * rt],dim = -1)
        ...
        hidden = cat([hidden,gru_hidden],dim = 0)
        return rnn_output, hidden

class LuongAttnDecoderRNN(Module):
    def __init__(self,embedding,static_emotion_embedding,emotion_embedding, hidden_size, output_size,device,ememory=None, n_layers=1, dropout=0.1,num_emotions = 7,batch_size = 64):
        ...
        self.generic_word_output_layer = Linear(self.hidden_size,output_size)
        # emotional output linear layer 
        self.emotion_word_output_layer = Linear(self.hidden_size,output_size)
        # emotional gate/ choice layer
        self.alpha_layer = Linear(hidden_size,1)
        # Luong eq 5 layer
        self.concat = Linear(hidden_size * 2, hidden_size)
    def forward(self, input_step, input_static_emotion, input_step_emotion, last_hidden
        ...
        '''
        if not is_floating_point(input_step_emotion):
        ...
        concat_input = cat((rnn_output, context), -1)
        ...
        if self.external_memory is not None:
            ...
            g = cat([(1 - g),g],dim = -1) # gate distribution
            g = g.squeeze(0)
        else:
...
inp = zeros((1,64,1500))
last_hidden = ones((2,64,500))
static_emo = randn((64,500))
internal_memory = randn((1,64,500))
...
def maskNLLLoss_IMemory(inp, target, mask,M_emo,external_memory,alpha):
    ...
    # cross entropy loss
    crossEntropy = -log(gather(inp, 1, target.view(-1, 1)).squeeze(1) + 1e-12)
    # internal emotional loss
    eos_mask = (target == 2) # 2 is EOS token
    eos_mask = eos_mask.type_as(M_emo)
    internal_memory_loss = norm(M_emo,dim = 2) * eos_mask
    internal_memory_loss = internal_memory_loss.squeeze(0)
    # external
    # find 1,0
    if external_memory is not None:
        qt = gather(external_memory.view(-1,1),0,target.view(-1,1)).type(torch.LongTensor)
        qt = qt.to(device)
        alpha_prob = gather(alpha,1,qt) # if it select emotion word or generic word
        external_memory_loss = (-log(alpha_prob + 1e-12)).reshape(-1) 
    else:
        external_memory_loss = zeros(crossEntropy.shape,dtype=torch.float,device=device)
...
def train(input_variable, lengths, target_variable,target_variable_emotion,
    ...
    context_input = zeros(num_samples,hidden_size,dtype=torch.float,device=device) #torch.FloatTensor(batch_size,hidden_size)
...
class GreedySearchDecoder(Module):
    ...
    def forward(self, input_seq,target_emotions,input_length, max_length):
        ...
        decoder_input = ones(1, 1, device=device, dtype=torch.long) * SOS_token
        # Initialize tensors to append decoded words to
        all_tokens = zeros([0], device=device, dtype=torch.long)
        all_scores = zeros([0], device=device)
        # Set initial context value,last_rnn_output, internal_memory
        context_input = zeros((1,hidden_size),dtype=torch.float,device=self.decoder.device)
...
class BeamSearchDecoder(Module):
    ...
    def forward(self, input_seq,target_emotions,input_length, max_length):
        ...
        # Initialize tensors to append decoded words to
        all_tokens = zeros([0], device=device, dtype=torch.long)
        all_words_order = zeros((1,self.num_word),device=decoder.device,dtype=torch.long)
        all_scores = zeros([0], device=device)
        all_scores_array = zeros((1,self.num_word),device=decoder.device,dtype=torch.float)
        # Set initial context value,last_rnn_output, internal_memory
        context_input = zeros(1,hidden_size,dtype=torch.float)
        ...
        for _ in range(max_length):
            ...
            all_tokens = cat((all_tokens, decoder_input), dim=0)
            all_scores = cat((all_scores, decoder_scores), dim=0)
            all_scores_array = cat((all_scores_array,decoder_output),dim = 0)
            all_words_order = cat((all_words_order,decoder_input_order), dim=0)
            # Prepare current token to be next decoder input (add a dimension)
            decoder_input = torch.unsqueeze(decoder_input, 0)
        # Return collections of word tokens and scores
